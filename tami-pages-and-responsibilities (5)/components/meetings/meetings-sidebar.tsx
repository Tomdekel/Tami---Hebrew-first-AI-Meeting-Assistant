...
import type { ProcessingStage } from "@/components/processing-stepper"
...
export function MeetingsSidebar({ meetings, selectedId, onSelect, onRetry }: MeetingsSidebarProps) {
        ...
        return (
          <div className="flex flex-col items-end gap-1">
            ...
            {meeting.processingStage && (
              <span className="text-[10px] text-muted-foreground">{getStageLabel(meeting.processingStage)}</span>
            )}
          </div>
        )
      case "draft":
  ...
  const getStageLabel = (stage: ProcessingStage) => {
    const labels: Record<ProcessingStage, { he: string; en: string }> = {
      uploading: { he: "מעלה", en: "Uploading" },
      transcribing: { he: "מתמלל", en: "Transcribing" },
      summarizing: { he: "מסכם", en: "Summarizing" },
    }
          ...
          {filteredMeetings.map((meeting) => (
            <button
              ...
              className={cn(
                ...
                selectedId === meeting.id ? "bg-teal-50 border border-teal-200" : "hover:bg-muted",
              )}
            >
