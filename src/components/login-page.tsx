...
import { Brain, Mail, Eye, EyeOff, Lock, Mic, Shield, MessageSquare, Loader2 } from "lucide-react"
...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(isRTL ? "ההתחברות נכשלה" : "Login failed", {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast.error(isRTL ? "התחברות Google נכשלה" : "Google sign-in failed", {
        description: error instanceof Error ? error.message : undefined,
      })
