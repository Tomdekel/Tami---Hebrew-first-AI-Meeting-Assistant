...
import { exchangeOutlookCode } from "@/features/meeting-bots/oauth/outlook";
...
export async function GET(request: NextRequest) {
  ...
  if (!state || state.provider !== "outlook" || state.userId !== user.id) {
    return NextResponse.json({ error: "Invalid OAuth state" }, { status: 400 });
  }

  try {
    const config = getOutlookOAuthConfig();
    const tokens = await exchangeOutlookCode(config, code);
    ...
    return NextResponse.redirect(new URL("/meetings/new?integration=outlook&connected=1", request.url));
  } catch (error) {
